...
export const FeedbackModal = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div 
        className="bg-gradient-to-br from-dark-card to-dark-secondary border border-primary/30 rounded-3xl p-8 max-w-2xl w-full shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-2xl font-black text-white mb-2">Enviá tu comentario</h2>
          ...
          <button
            onClick={onClose}
            className="text-dark-muted hover:text-white transition text-2xl leading-none"
          >
            ×
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 bg-dark-secondary border border-dark-border text-dark-muted font-bold py-3 px-6 rounded-lg hover:bg-dark-border transition-all"
            >
              Cancelar
            </button>
            <button
