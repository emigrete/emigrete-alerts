...
const TWITCH_CLIENT_ID = "wyx70txe25e9xqb3bv7fmd09fk2aaq";
// ---------------------
...
const styles = {
    ...
    tr: { background: '#202023', transition: 'transform 0.1s' },
...
export default function Dashboard() {
    ...
    const scopes = encodeURIComponent("channel:read:redemptions channel:manage:redemptions");
    const redirect = encodeURIComponent(REDIRECT_URI);
    const loginUrl = `https://id.twitch.tv/oauth2/authorize?client_id=${TWITCH_CLIENT_ID}&redirect_uri=${redirect}&response_type=code&scope=${scopes}&force_verify=true`;
            ...
            <header style={styles.header}>
                <div>
                    ...
                    <div style={{marginTop: 5, display: 'flex', alignItems: 'center', gap: 10}}>
                        <span style={{width: 8, height: 8, borderRadius: '50%', background: theme.success}}></span>
                        <span style={{color: theme.textMuted, fontSize: '0.85rem', fontFamily: 'monospace'}}>Usuario: {userId}</span>
                    </div>
                </div>
                <button onClick={() => {localStorage.removeItem('twitchUserId'); setUserId(null);}} style={styles.btnLogout}>
                    Cerrar Sesión
                </button>
            </header>
                            ...
                            <div style={styles.fileInputWrapper}>
                                ...
                                <label htmlFor="video-upload" style={styles.fileLabel(!!fileError)}>
                                    ...
                                    {file ? (
                                        ...
                                        <span>Arrastrá o clickeá para subir video<br/><small style={{color: theme.textMuted}}>Max 5MB (.mp4, .webm)</small></span>
                                    )}
                                </label>
                            </div>
                    ...
                    {triggers.length === 0 ? (
                        ...
                        <div style={{overflowX: 'auto'}}>
                            <table style={styles.table}>
                                <thead>
                                    <tr>
                                        <th style={{...styles.th, borderTopLeftRadius: 8}}>Recompensa</th>
                                        <th style={styles.th}>Link para OBS</th>
                                        <th style={{...styles.th, textAlign: 'right', borderTopRightRadius: 8}}>Acciones</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {triggers.map(t => {
                                        ...
                                        return (
                                            <tr key={t._id}>
                                                <td style={{...styles.td, borderTopLeftRadius: 8, borderBottomLeftRadius: 8}}>
                                                    <strong style={{color: theme.primary, fontSize: '1rem'}}>
                                                        {rewards.find(r => r.id === t.twitchRewardId)?.title || "Desconocido"}
                                                    </strong>
                                                ...
                                                <td style={styles.td}>
                                                    <div style={styles.codeBox}>
                                                        ...
                                                        <button 
                                                            ...
                                                            style={{background: 'none', border: 'none', color: theme.textMuted, cursor: 'pointer', transition: 'color 0.2s'}}
                                                            title="Copiar Link"
                                                        >
                                                ...
                                                <td style={{...styles.td, textAlign: 'right', borderTopRightRadius: 8, borderBottomRightRadius: 8}}>
                ...
                <div style={{...styles.card, border: '1px solid #3f3f46', background: 'linear-gradient(45deg, #18181b 0%, #1c1c21 100%)'}}>
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 20}}>
                        ...
                        <div style={styles.donationContainer}>
                            {/* CAFECITO (ARS) */}
                            <a href="https://cafecito.app/emigrete" target="_blank" rel="noopener noreferrer" style={styles.btnDonation(theme.cafecito)} title="Solo Argentina">
                                <span>☕</span> Cafecito 🇦🇷
                            </a>

                            {/* PAYPAL (USD) */}
                            <a href="https://www.paypal.me/teodorow" target="_blank" rel="noopener noreferrer" style={styles.btnDonation(theme.paypal)} title="Internacional">
                                <span>🌍</span> PayPal
                            </a>
                        </div>
